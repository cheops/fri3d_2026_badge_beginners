...
class Main(Activity):
    ...
    def onCreate(self):
        ...
        button_label.center()
        button.align_to(hue_slider, lv.ALIGN.OUT_BOTTOM_MID, -60, 20)
        ...
        button_chase = lv.button(screen)
        button_chase.set_size(54, 32)
        button_chase_label = lv.label(button_chase)
        button_chase_label.set_text("chase")
        button_chase_label.center()
        button_chase.align_to(hue_slider, lv.ALIGN.OUT_BOTTOM_MID, 60, 20)
        def button_chase_clicked(e):
            r, g, b = self.colorHSV(self._hue, 255, self._slider_value)
            for i in range(15):
                LightsManager.clear()
                ...
                LightsManager.write()
                time.sleep_ms(50)
            self.update_leds()  # restore all LEDs to the current color
        button_chase.add_event_cb(button_chase_clicked, lv.EVENT.CLICKED, None)
